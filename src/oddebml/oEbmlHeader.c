                              ...
                              cErrorStack es[static 1] )
{
   ...
   if ( not scan_ebml_uint_element_o( sca, &(header->version) ) )
      return push_imp_ebml_error_o( es, sca, O_EbmlVersion.id, o_EbmlUint );
   ...
   if ( not scan_ebml_uint_element_o( sca, &(header->readVersion) ) )
      return push_imp_ebml_error_o( es, sca, O_EbmlReadVersion.id, o_EbmlUint );
   ...
   if ( not scan_ebml_uint_element_o( sca, &(header->maxIdLength) ) )
      return push_imp_ebml_error_o( es, sca, O_EbmlMaxIdLength.id, o_EbmlUint );
   ...
   if ( not scan_ebml_uint_element_o( sca, &(header->maxSizeLength) ) )
      return push_imp_ebml_error_o(
                es, sca, O_EbmlMaxSizeLength.id, o_EbmlUint
             );
   ...
   if ( not view_ebml_string_element_o( sca, &(header->docType) ) )
      return push_imp_ebml_error_o( es, sca, O_EbmlDocType.id, o_EbmlString );
   ...
   if ( not scan_ebml_uint_element_o( sca, &(header->docTypeVersion) ) )
      return push_imp_ebml_error_o(
                es, sca, O_EbmlDocTypeVersion.id, o_EbmlUint
             );
   ...
   if ( not scan_ebml_uint_element_o( sca, &(header->docTypeReadVersion) ) )
      return push_imp_ebml_error_o(
                es, sca, O_EbmlDocTypeReadVersion.id, o_EbmlUint
             );
